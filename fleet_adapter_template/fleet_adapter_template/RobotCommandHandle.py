...
class RobotCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
        ...
        assert self.starts, (
            f"Unable to determine StartSet for robot:{self.name}")
        start = self.starts[0]
              ...
              self.state == RobotState.WAITING):
                ...
                if self.state == RobotState.IDLE:
                    ...
                    [x, y] = self.transforms["rmf_to_robot"].transform(
                        target_pose[:2])
                    theta = math.degrees(
                        target_pose[2] + self.transforms['orientation_offset'])
                    # ------------------------ #
                    ...
                    if response:
                        ...
                        with self._lock:
                            # robot starts at a graph waypoint
                            if self.on_waypoint is not None:
                                self.last_known_waypoint_index = \
                                  self.on_waypoint
                                target_index = self.target_waypoint.graph_index
                                # update the lane on which the robot will move
